...
type Props = {
  ...
  mode: 'edit' | 'add';
  onClose: () => void;
  onUpdate: (data: JobDetailsDto) => Promise<void>;
};
const JobForm = ({ jobId, mode, onClose, onUpdate }: Props) => {
  console.log('from edit job page', jobId);
  ...
  const {
    ...
    error,
    setError,
  } = useJobs(mode, jobId, onClose, onUpdate);
  ...
  useEffect(() => {
    ...
    const getJob = async () => {
      ...
      console.log('after getting job for edit ', data.jobDetails);

      setFormData({
        ...data.jobDetails,
        country: data.jobDetails.location.country,
        state: data.jobDetails.location.state,
        lastDate: data.jobDetails.lastDate
          ? new Date(data.jobDetails.lastDate).toISOString().slice(0, 10)
          : '',
        vacancyCount: data.jobDetails.vacancyCount.toString(),
        min_salary: data.jobDetails.min_salary.toString(),
        max_salary: data.jobDetails.max_salary.toString(),
      });
    };
