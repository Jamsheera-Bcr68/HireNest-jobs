...
export default function EditCompanyProfileModal({
  ...
  const handleChange = (
    ...
    } else if (section == 'socialMediaLinks') {
      setFormData((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    if (company) {
      ...
      setFormData((prev) => ({
        ...prev,
        startedIn: company.startedIn.toString(),
      }));
    }
  }, [company]);
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    console.log('from form data submit', formData);
    const result = companyProfileEditSchema.safeParse(formData);
    if (!result.success) {
      const err = result.error.format();
      console.log('validateion eerror', err);

      const formattedError: FormErrors = {
        size: err?.size?._errors?.[0] || '',
        companyName: err?.companyName?._errors?.[0] || '',
        tagLine: err?.tagLine?._errors?.[0] || '',

        startedIn: err?.startedIn?._errors?.[0] || '',
        industry: err?.industry?._errors?.[0] || '',
        website: err?.website?._errors?.[0] || '',
        address: {
          state: err?.address?.state?._errors?.[0] || '',
          country: err?.address?.country?._errors?.[0] || '',
          place: err?.address?.place?._errors?.[0] || '',
        },
        socialMediaLinks: {
          linkedIn: err?.socialMediaLinks?.linkedIn?._errors?.[0] || '',
          whatsapp: err?.socialMediaLinks?.whatsapp?._errors?.[0] || '',
          youtube: err?.socialMediaLinks?.youtube?._errors?.[0] || '',
          twitter: err?.socialMediaLinks?.twitter?._errors?.[0] || '',
          gitHub: err?.socialMediaLinks?.gitHub?._errors?.[0] || '',
          portfolio: err?.socialMediaLinks?.portfolio?._errors?.[0] || '',
        },
      };
    ...
    try {
      const data = await companyService.editProfile(result.data);
      console.log('after update company', data);
            ...
            <div className="mt-6 space-y-3 ml-2">
              ...
              <div className="flex gap-1">
                <div className="flex-1">
                  ...
                  {error?.industry && (
                    <p className="text-red-500 text-sm">*{error.industry}</p>
                  )}
                </div>
                <div className="flex-1">
                  <label
                  ...
                  {error?.size && (
                    <p className="text-red-500 text-sm">*{error.size}</p>
                  )}
                </div>
              </div>
              <div>
                ...
                <div>
                  ...
                  {error?.startedIn && (
                    <p className="text-red-500 text-sm">*{error.startedIn}</p>
                  )}
                </div>
              </div>
              <div>
